...
import colorSx from "./color.module.css"
...
export const ColorButton = memo(({ color, active, onColorChange }: ColorButtonProps) => {
  ...
  return (
    <button onClick={handleClick} className={colorSx.container}>
      <div style={{ backgroundColor: color, color: color }} className={colorSx.bubble}></div>
    </button>
  )
})
